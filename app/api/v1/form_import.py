...
from app.api.deps import get_excel_import_service
...
async def _read_xlsx_upload(file: UploadFile) -> bytes:
    """Validate the upload is an .xlsx file and return its bytes, or raise 400."""
...
async def _parse_upload(file: UploadFile, service: ExcelImportService):
    """Shared parse flow: read, parse, and return the success or 400 error response."""
    ...
    if not success:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content=ImportErrorResponse(errors=errors).model_dump(),
        )
...
async def import_from_excel(
    ...
    **Validacije:**
    ...
    """
    return await _parse_upload(file, service)
